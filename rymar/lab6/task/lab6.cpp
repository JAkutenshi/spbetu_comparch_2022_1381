...
void res_func2(int Xmax, int* ArrLeftBorder, int* ArrNumber, int QuantNumber, int* ArrAverage);
ofstream fout("result.txt");

extern "C"
{
	void func1(int ArrNumber[], int QuantNumber, int ArrInter1[], int Xmin);
	void func2(int ArrWith1Range[], int ArrLeftBorder[], int ArrInterDif[], int ArrAverage[], int ArrSum[], int QuantBorder, int Xmin);
}
...
int main(void) {
	...
	int* ArrAverage = NULL; // для задания на защиту
	int* ArrSum = NULL; // массив сумм
	...
	ArrAverage = new int[QuantBorder] {0}; // выделяем память под массив ср ариф
	ArrSum = new int[QuantBorder] {0}; // выделяем память под массив сумм
	func1(ArrNumber, QuantNumber, ArrInter1, Xmin); //распределение по ед. интервалам 
	func2(ArrInter1, ArrRightBorder, ArrInterDif, ArrAverage, ArrSum, QuantBorder, Xmin); //распределение по опр. интервалам 
	for (int i = 0; i < QuantBorder; i++) {
		cout << ArrSum[i] << '\n';
	}
	res_func1(Xmin, ArrInter1, MainRange); 	//вывод результата первой процедуры
	res_func2(Xmax, ArrLeftBorder, ArrInterDif, QuantBorder, ArrAverage); 	//вывод результата второй процедуры
...
void res_func2(int Xmax, int* ArrLeftBorder, int* ArrInterDif, int QuantNumber, int* ArrAverage)
{

	cout << "\nРаспределение случайных чисел по заданным интервалам\n"; 
	fout << "\nРаспределение случайных чисел по заданным интервалам\n";
	cout << "№\t[левая;правая гр]\tКоличество\tСреднее значение\n"; 
	fout << "№\t[левая;правая гр]\tКоличество\tСреднее значение\n";
	for (int i = 0; i < QuantNumber; i++)
	{
		...
		cout << "\t\t" << ArrInterDif[i] << "\t\t" << ArrAverage[i] << '\n';
		fout << "\t\t" << ArrInterDif[i] << "\t\t" << ArrAverage[i] << '\n';
