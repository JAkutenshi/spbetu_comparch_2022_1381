...
void generate(int*& arr, int NumRamDat, int min, int max);
...
void receive(int& NumRamDat, int*& arr, int& min, int& max, int& NInt, int*& LGrInt) {
    ...
    std::cout << "Please enter the limits of random numbers:" << std::endl << "Min:";
    std::cin >> min;
    std::cout << "Max:";
    std::cin >> max;
    while (min >= max) {
        std::cout << "Invalid Xmax, please enter Xmax again:";
        std::cin >> max;
    }
    generate(arr, NumRamDat, min, max);
    std::cout << "Please enter the number of intervals:";
    std::cin >> NInt;
...
void help_print(int min, int max, int* arr) {
    std::cout << "Intermediate result: " << std::endl;
    file << "Intermediate result: " << std::endl;
    std::cout << "Index\t" << "Border\t" << "Count" << std::endl;
    file << "Index\t" << "Border\t" << "Count" << std::endl;
    int k = 0;
    for (int i = min; i <= max; i++) {
        std::cout << "  " << k + 1 << "\t  " << i << "\t  " << arr[k] << std::endl;
        file << "  " << k + 1 << "\t  " << i << "\t  " << arr[k] << std::endl;
        k++;
    }
}
...
int main()
{
   ...
   /* for (int i = 0; i < Xmax - Xmin + 1; i++) {
        std::cout << answer_arr[i] << " ";
    }*/
    help_print(Xmin, Xmax, answer_arr);
